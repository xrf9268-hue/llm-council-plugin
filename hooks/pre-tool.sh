...
validate_council_operation() {
    ...
    # Build absolute path using three-level fallback chain
    # 1. COUNCIL_PLUGIN_ROOT (set by SessionStart hook)
    # 2. CLAUDE_PLUGIN_ROOT (marketplace installations)
    # 3. CLAUDE_PROJECT_DIR (local development)
    local abs_path=""
    if [[ -n "${COUNCIL_PLUGIN_ROOT:-}" ]]; then
        abs_path="${COUNCIL_PLUGIN_ROOT}/${script_path}"
    ...
    if [[ ! -f "$abs_path" ]]; then
        ...
        echo "Debug: COUNCIL_PLUGIN_ROOT=${COUNCIL_PLUGIN_ROOT:-<not set>}" >&2
